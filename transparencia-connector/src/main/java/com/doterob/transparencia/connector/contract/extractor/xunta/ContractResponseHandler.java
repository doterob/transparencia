 ...
 */
public class ContractResponseHandler implements ResponseHandler<List<Contract>> {

    @Override
    public List<Contract> handleResponse(HttpResponse response) throws ClientProtocolException, IOException {

        final Map<Integer, Contract> result = new HashMap<Integer, Contract>();
        ...
        for(int i = 1; i < doc.getElementById("tabs-4").select("td.c65").size(); i++) {
            ...
            result.put(lot, new Contract(code, date, subject, contractorId, contractorName, area, amount));
        }
